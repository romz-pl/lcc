#include <lcc/alloc/SmallVector.h>
...
void *SmallVectorBase<Size_T>::replaceAllocation(void *NewElts, size_t TSize,
                                                 size_t NewCapacity,
                                                 size_t VSize) {
  void *NewEltsReplace = lcc::safe_malloc(NewCapacity * TSize);
  if (VSize)
...
void *SmallVectorBase<Size_T>::mallocForGrow(void *FirstEl, size_t MinSize,
  ...
  void *NewElts = lcc::safe_malloc(NewCapacity * TSize);
  if (NewElts == FirstEl)
...
void SmallVectorBase<Size_T>::grow_pod(void *FirstEl, size_t MinSize,
  ...
  void *NewElts;
  if (BeginX == FirstEl) {
    NewElts = lcc::safe_malloc(NewCapacity * TSize);
    if (NewElts == FirstEl)
      NewElts = replaceAllocation(NewElts, TSize, NewCapacity);
    ...
    NewElts = lcc::safe_realloc(this->BeginX, NewCapacity * TSize);
    if (NewElts == FirstEl)
...
template class lcc::SmallVectorBase<uint32_t>;
...
template class lcc::SmallVectorBase<uint64_t>;
