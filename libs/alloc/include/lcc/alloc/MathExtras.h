...
unsigned countTrailingZeros(T Val, ZeroBehavior ZB = ZB_Width) {
  static_assert(std::is_unsigned_v<T>,
                "Only unsigned integral types are allowed.");
  return lcc::detail::TrailingZerosCounter<T, sizeof(T)>::count(Val, ZB);
}
...
unsigned countLeadingZeros(T Val, ZeroBehavior ZB = ZB_Width) {
  static_assert(std::is_unsigned_v<T>,
                "Only unsigned integral types are allowed.");
  return lcc::detail::LeadingZerosCounter<T, sizeof(T)>::count(Val, ZB);
}
...
template <size_t kValue> constexpr inline size_t CTLog2() {
  static_assert(kValue > 0 && lcc::isPowerOf2_64(kValue),
...
inline uint64_t DoubleToBits(double Double) {
  static_assert(sizeof(uint64_t) == sizeof(double), "Unexpected type sizes");
  return std::bit_cast<uint64_t>(Double);
}
...
inline uint32_t FloatToBits(float Float) {
  static_assert(sizeof(uint32_t) == sizeof(float), "Unexpected type sizes");
  return std::bit_cast<uint32_t>(Float);
}
