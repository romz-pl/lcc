...
extern template class lcc::SmallVectorBase<uint32_t>;
#if SIZE_MAX > UINT32_MAX
extern template class lcc::SmallVectorBase<uint64_t>;
#endif
...
namespace std {
  ...
  template<typename T>
  inline void
  swap(lcc::SmallVectorImpl<T> &LHS, lcc::SmallVectorImpl<T> &RHS) {
    LHS.swap(RHS);
  }
  ...
  template<typename T, unsigned N>
  inline void
  swap(lcc::SmallVector<T, N> &LHS, lcc::SmallVector<T, N> &RHS) {
    LHS.swap(RHS);
  }
